...
def run_detection(image_path, weights_path='./yolov9/yolov9-e.pt', img_size=640, device=0, classes=[0], output_name='yolov9_c_c_640_detect_all', model_idx=0):
    ...
    try:
        ...
        # Load model
        stride, names, pt = models[model_idx][0].stride, models[model_idx][0].names, models[model_idx][0].pt
        ...
        for path, im, im0s, _, _ in dataset:
            ...
            im = im.half() if models[model_idx][0].fp16 else im.float()  # uint8 to fp16/32
            ...
            pred_all = []
            start_time = time.time()
            for model in models[model_idx]:  # Sequential inference on all models
                pred = model(im)
...
def process_image(image_data, model_idx):
    ...
    while retry_count < 5:  # Retry up to 5 times
        ...
        if used / total < 0.9:
            result = run_detection(image_path, weights_path, img_size, device, classes, output_name, model_idx)
...
def worker():
    while True:
        try:
            ...
            with condition:
                while not any(model_availability):
                    print("All models are busy. Waiting for a model to become free...")
                    condition.wait()  # Wait for a model to become free

                # Find the first available model
                model_idx = model_availability.index(True)
                model_availability[model_idx] = False  # Mark the model as busy

            # Process the image with the selected model
            result = process_image(image_data, model_idx)

            with condition:
                model_availability[model_idx] = True  # Mark the model as free
                condition.notify_all()  # Notify all waiting workers
...
async def on_startup():
    ...
    workers = start_worker_pool(num_workers=20)
...
if __name__ == "__main__":
    ...
    # Shared state for model availability
    manager = multiprocessing.Manager()
    model_availability = manager.list([True] * 20)  # Assuming 20 models
    condition = multiprocessing.Condition()

    weights = ['./yolov9/yolov9-e.pt' for w in range(20)]

    device = select_device('0')
    models_ = [DetectMultiBackend(w, device=device) for w in weights]  # Load all 20 models
    count = 0
